...
class __Home_Controller extends Controller
{
    ...
    public function __Search_index()
    {
        return view('__search__page');
    }
    public function __Search__($id, $category)
    {
        $query = $category == 'categories' ? CategoriesModel::query() : ListsModel::query();
        $query->where('title', 'LIKE', '%' . $id . '%');
        return response()->json([
            'data' => $query->get(),
        ]);
    }
    public function __Search__Edit__($id, $category)
    {
        $query = $category == 'categories' ? CategoriesModel::query() : ListsModel::query();
        $query->where('id', $id);
        return response()->json([
    ...
    public function __Search__Update__(Request $request)
    {

        $update = $request->categoriesId == 'categories' ? CategoriesModel::find($request->dataId) : ListsModel::find($request->dataId);
        $__upload__status__ = true;
        if ($request->file('file') != null) {
            $file = $request->file('file');
            $fileName = time() . '_' . $file->getClientOriginalName();
        ...
        if ($__upload__status__) {
            ...
            if ($request->SearchTitleUpdatingSearch != null) {
                $update->title = $request->SearchTitleUpdatingSearch;
            }
            if ($request->SearchContentHeaderUpdating) {
                $update->contentHeader = $request->SearchContentHeaderUpdating;
            }
            if ($request->SearchContentBody) {
                $update->contentBody = $request->SearchContentBody;
            }
            if ($request->SearchPriceUpdating) {
                $update->price = $request->SearchPriceUpdating;
            }
            if ($request->file('file') != null) {
                $update->image = $fileName;
            }
        ...
        // return $request->SearchTitleUpdatingSearch;
    }
    public function __Search__Delete__($id, $cate)
    {
        $update = $cate == 'categories' ? CategoriesModel::find($id) : ListsModel::find($id);

        if ($update->delete()) {
            return back()->with([
                'message' => $cate . 'Başarıyla Güncellendi',
                'style' => 'width:90%;height:80%;font-size:20px;border-radius:10px;color:#fff;background: #0E6655;display:flex;justify-content: center;align-items: center;'
            ]);
        } else {
            return back()->with([
                'message' => $cate . 'Güncelleme Başarısız',
                'style' => 'width:90%;height:80%;font-size:20px;border-radius:10px;color:#fff;background: #C0392B;display:flex;justify-content: center;align-items: center;'
            ]);
        }
    }
